...
input int    InpDelayMinutes = 240; // delay between trades in minutes
...
bool TradeOpen( ENUM_ORDER_TYPE type ) {
   ...
   if ( TimeCurrent() < NextTradeTime ) return ( false );
   ...
   if ( result ) {
      NextTradeTime = TimeCurrent() + ( InpDelayMinutes * 60 );
   }
